...
列出当前 Scope 下的每一件资源（不止虚拟机），支持按名称/类型/资源组/区域搜索。可以勾选多项、
点击「删除所选」一次性清理；虚拟机行不提供删除（虚拟机有自己专门的删除流程，见上文），只提供
「查看虚拟机」跳转。

### 资源组

按资源组查看，显示每个资源组内的资源数量。删除资源组是**级联删除**——组内所有资源都会被一并
删除，破坏面比删除单个资源大得多，因此确认框要求**输入资源组名称本身**才能继续（对齐 Azure
门户自己删除资源组的交互）。同样支持勾选多个资源组批量删除。

批量删除时，所有目标会先分析影响、合并成一个确认框统一展示，确认后逐项执行并显示进度；中途
取消会作废这一批还没执行的操作，不会遗留一堆"半途而废"的待处理项。
