...
取令牌、列订阅这类只读操作遇到**临时网络波动**（连接被重置、代理握手失败、DNS 解析失败等）时，
CloudFlow 会自动重试（最多 3 次、指数退避），大多数瞬时波动无需人工处理即可自愈。重试耗尽后
提示的是一句可行动的「网络连接中断…请检查网络连接（或代理）后重试」，而不是底层错误细节。

需要注意的两点边界：

- 启动时若恰逢网络瞬断导致账户暂时不可用，CloudFlow **不会**移除该账户的登记——账户登录状态
  保留在本机，界面退回演示数据，网络恢复后重新启动应用即可自动接回，无需重新登录。
- 只有确认是账户本身失效（如 Profile 目录被手动删除）才会清除登记；此时界面会明确告知
  「上次使用的账户已不可用」。
...
| 个人账户登录成功但看不到任何订阅 | 该 Microsoft 账户下没有可管理的 Azure 订阅，或订阅权限不在
  这个账户名下 |
| 提示「网络连接中断，无法获取访问令牌」 | 已自动重试仍失败的持续网络问题；检查本机网络或代理
  是否能访问 `login.microsoftonline.com`，恢复后重新启动应用即可——账户登记不会因此丢失 |
