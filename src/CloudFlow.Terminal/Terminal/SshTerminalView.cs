...
public sealed class SshTerminalView : ContentControl, IDisposable
{
    ...
    private async void OnLoaded(object sender, RoutedEventArgs e)
    {
        ...
        catch (Exception ex)
        {
            ...
            // 也不知道重新点「连接」就能换一个全新会话重试。失败原因带上一线异常的具体
            // 内容（资源缺失 / 释放失败 / WebView2 起不来各有各的说法），同时写进
            // 会话（标签条显示「连接失败」+ 悬停可见原因）与终端本体（渲染器若起得来才可见）。
            _session.MarkStartFailed(
                $"终端组件初始化失败：{ex.Message} 请关闭该会话后重新连接重试；"
                + "若持续失败，请确认本机已安装 Microsoft Edge WebView2 运行时。");
            SetStateLine($"终端组件初始化失败：{ex.Message}");
        }
    }
